...
size_l, name_l, ls, dtd_vers, wds = 0, 0, [], [], [[], []] # 变量：list, msize/lname_lengths
	...
	for w in wds[i]:
		if is_mcworld(f'{get_wrlz(i)}\\{w}') == True:
			with open(f'{get_wrlz(i)}\\{w}\\levelname.txt', 'r', encoding = 'UTF-8') as l:
				...
				if get_width(name) > 14:
					while get_width(name) > 11: name = name[:-1]
					name += '...'
...
安装MCBE：     https://mcappx.com      MCLauncher使用教程：https://github.com/UMSCJK/MCLauncher-tutorial
''')
		...
		for i in range(0, len(ls)):
			...
			name_s, size_s = ' ' * (name_l - get_width(ls[i][6])), ' ' * (size_l - len(str(ls[i][5])))
			world_ls += f'| {idex_s}{i + 1} | {vers[ls[i][0]]} | {ls[i][6]}{name_s}'
			world_ls += f' | {ls[i][1]} | {size_s}{ls[i][5]} MB | {ls[i][3]} |\n'
		print(f'''╭{'─' * (l_sum + 54)}╮\n| {title} - 检测到：{dtd_str}{' ' * (l_sum - len(dtd_vers) * 8 + 28)}|
├─{'─' * (idex_l - 4)}─────┬────────┬─────{'─' * (name_l - 4)}─┬──────────────┬─{'─' * (size_l - 1)}─────┬──────────────────┤
| {' ' * (idex_l - 4)}编号 |  版本  | 名称{' ' * (name_l - 4)} | 存档文件夹名 | {' ' * (size_l - 1)}大小 | 修改时间         |
| {'-' * (idex_l - 4)}---: | :----: | :---{'-' * (name_l - 4)} | ------------ | {'-' * (size_l - 1)}---: | ---------------- |
{world_ls}├─{'─' * idex_l}─┴────────┴─────{'─' * (name_l - 4)}─┴──────────────┴─{'─' * (size_l - 1)}─────┴──────────────────┤
| 选项【0】退出【o/p】打开正式版/预览版存档{' ' * (l_sum + 11)} |
|【编号】查看存档详细信息【编号+e】导出存档{' ' * (l_sum + 11)} |\n╰{'─' * (l_sum + 54)}╯''')
		sel = input(prompt)
		if sel == '0': exit()
		elif sel == '': list_all()
		elif sel.lower() == 'o': opendir(get_wrlz(0)); list_all()
		elif sel.lower() == 'p': opendir(get_wrlz(1)); list_all()
		elif sel.lower() in ['op', 'po']: opendir(get_wrlz(0)); opendir(get_wrlz(1)); list_all()
		elif sel.isdigit() == True:
			if int(sel) <= len(ls): show_info(int(sel) - 1)
